...
export async function getUserPreferences(userId: string): Promise<UserPreferences | null> {
  ...
  const rows = await db()`SELECT body_model,target_weight,weekly_goal,show_insights,show_advanced,show_history,configured FROM user_preferences WHERE user_id=${userId}` as unknown as Record<string, unknown>[];
  const row = rows[0];
  if (!row) return null;
  return {
...
export async function saveUserPreferences(userId: string, encryptedApiKey: string, input: Partial<UserPreferences>) {
  ...
  await db()`INSERT INTO user_preferences (user_id,encrypted_api_key,body_model,target_weight,weekly_goal,show_insights,show_advanced,show_history,configured)
    VALUES (${userId},${encryptedApiKey},${bodyModel},${targetWeight},${weeklyGoal},${input.showInsights !== false},${input.showAdvanced !== false},${input.showHistory !== false},true)
    ON CONFLICT(user_id) DO UPDATE SET encrypted_api_key=excluded.encrypted_api_key,body_model=excluded.body_model,target_weight=excluded.target_weight,weekly_goal=excluded.weekly_goal,show_insights=excluded.show_insights,show_advanced=excluded.show_advanced,show_history=excluded.show_history,configured=true,updated_at=now()`;
  return getUserPreferences(userId);
}
